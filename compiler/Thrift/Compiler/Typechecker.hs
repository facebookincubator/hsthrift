...
filterDecls reqSymbols symbolMap =
  ...
  where
    ...
    filterDecl (D_Service s, _, _) symbols = mkVertex newDecl
      where
        newDecl = D_Service s { serviceFunctions = filteredFuns }
        filteredFuns =
          filter (\Function{..} -> Set.member funName symbols) $
          serviceFunctions s
    filterDecl node _ = node
    ...
    constValSymbols IntVal{} = []
    constValSymbols DoubleVal{} = []
    constValSymbols StringVal{} = []
    constValSymbols BoolVal{} = []
    constValSymbols (IdVal name) = [name]
    constValSymbols ListVal{..} = concatMap (constSymbols . leElem) lvElems
    constValSymbols MapVal{..} =
...
resolveField sAnns sname field@Field{..} = do
  when (fieldId == 0) $
    typeError (lLocation $ flId fieldLoc) $ InvalidFieldId fieldName 0
  This ty <- resolveType fieldType
  ...
  case resolveTag fieldTag ty of
    ...
    Just tag ->
      return Field
        { fieldResolvedName = renameField options sAnns sname field
...
resolveFunction f@Function{..} = do
  (rtype, args, excepts)
    <- (,,) <$> sequence ((`withSome` resolveType) <$> rightToMaybe funType)
...
mkConstMap (thriftName, opts@Options{..}) imap tmap = foldM insertC emptyContext
  where
    ...
    insertC m (D_Const Const{..}) = do
      ty <- runTypechecker env $ resolveType constType
...
typecheckConst I8  (UntypedConst _ (IntVal i _)) = literal $ fromIntegral i
typecheckConst I16 (UntypedConst _ (IntVal i _)) = literal $ fromIntegral i
typecheckConst I32 (UntypedConst _ (IntVal i _)) = literal $ fromIntegral i
typecheckConst I64 (UntypedConst _ (IntVal i _)) = literal $ fromIntegral i

-- Floating Point Types
typecheckConst TDouble (UntypedConst _ (IntVal i _)) = literal $ fromIntegral i
typecheckConst TDouble (UntypedConst _ (DoubleVal d _)) = literal d
typecheckConst TFloat (UntypedConst _ (IntVal i _)) = literal $ fromIntegral i
typecheckConst TFloat (UntypedConst _ (DoubleVal d _)) =
  literal $ double2Float d

-- Other Base Types
typecheckConst TBool (UntypedConst _ (IntVal i _))
  | i == 0 = literal False
  | i == 1 = literal True
typecheckConst TBool (UntypedConst _ (BoolVal b)) = literal b
typecheckConst TText (UntypedConst _ (StringVal s _)) = literal s
typecheckConst TBytes (UntypedConst _ (StringVal s _)) = literal $ encodeUtf8 s

-- Recursive Types
typecheckConst (TList u) (UntypedConst _ ListVal{..}) =
  Literal . List <$> mapT (typecheckConst u . leElem) lvElems
typecheckConst (TSet u) (UntypedConst _ ListVal{..}) =
  Literal . Set <$> mapT (typecheckConst u . leElem) lvElems
typecheckConst ty@(TSet _) c@(UntypedConst l MapVal{mvElems=[]}) = do
  Options{..} <- asks options
  if optsLenient then
    return . Literal $ Set [] -- weird files use the wrong empty brackets, sigh
  else
    typeError (lLocation l) (LiteralMismatch ty c)
typecheckConst (THashSet u) (UntypedConst _ ListVal{..}) =
  Literal . HashSet <$> mapT (typecheckConst u . leElem) lvElems
typecheckConst (TMap kt vt) (UntypedConst _ MapVal{..}) =
  Literal . Map <$> mapTWeird tcConsts mvElems
    where
      tcConsts ListElem{leElem=MapPair{..}} = (,)
        <$> typecheckConst kt mpKey
        <*> typecheckConst vt mpVal
typecheckConst (THashMap kt vt) (UntypedConst _ MapVal{..}) =
...
typecheckConst e@(TEnum Name{..} _loc _)
               lit@(UntypedConst Located{..} (IntVal i _)) = do
...
typecheckConst enum@(TEnum name _loc _)
               (UntypedConst Located{..} (IdVal ident)) = do
...
typecheckConst newt@(TNewtype _ ty _loc) val@(UntypedConst Located{..} c) =
   case c of
     ...
     -- however IdVals can also be enums, so we have to check for this case too
     IdVal ident ->
...
typecheckConst (TStruct Name{..} _loc)
               (UntypedConst Located{..} MapVal{..}) = do
  This schema <- lookupSchema sourceName lLocation
  Literal . This <$> (typecheckStruct lLocation schema =<< mkFieldMap mvElems)
typecheckConst (TException Name{..} _loc)
               (UntypedConst Located{..} MapVal{..}) = do
  This schema <- lookupSchema sourceName lLocation
  Literal . This . EV <$>
    (typecheckStruct lLocation schema =<< mkFieldMap mvElems)
typecheckConst (TUnion n@Name{..} _loc) (UntypedConst uloc MapVal{..}) = do
  This schema <- lookupUnion sourceName (lLocation uloc)
  ...
  fname <-
    case utname of
      UntypedConst _ (StringVal s _) -> pure s
      v@(UntypedConst utloc _) -> typeError (lLocation utloc) $ InvalidField v
  Literal . This <$> typecheckUnion schema fname val

-- Identifiers (typecheckIdentNum is not first parameter to the orT)
typecheckConst ty (UntypedConst Located{..} (IdVal ident)) =
...
typecheckIdent ty loc ident = do
  name <- mkThriftName ident
  (This trueTy, renamed, locDefined) <- lookupConst name loc
  Env{..} <- ask
  case trueTy `eqOrAlias` ty of
...
mkFieldMap = fmap Map.fromList . mapT getName
  where
    getName ListElem{ leElem = MapPair{..} } = case mpKey of
      UntypedConst _ (StringVal s _) -> pure (s, mpVal)
...
-- Default Field
typecheckStruct loc (SField proxy fname ty defVal schema) fields =
  case Map.lookup fname fields of
    Nothing  -> ConsDefault proxy ty
                <$> sequence (typecheckConst ty <$> defVal)
                <*> typecheckStruct loc schema fields
...
mkTypemap (thriftName, opts@Options{..}) imap =
  ...
  where
    ...
    resolve m (D_Typedef t@Typedef{..}) = do
      tdef <- mkTypedef <$> runTypechecker env (resolveType tdType)
...
resolveType AnnotatedType{..} =
  case atType of
    ...
    TMap k v -> do
      This rk <- resolveType k
      This rv <- resolveType v
      annotate $ TMap rk rv
    THashMap k v -> do
      This rk <- resolveType k
      This rv <- resolveType v
      annotate $ THashMap rk rv
  ...
  where
    ...
    resolve
      ...
      -> AnnotatedType Loc u
      -> TC l (Some (Type l))
    resolve mkType u = resolveType u >>= \(This v) -> annotate (mkType v)
      ...
      case (someSymbolVal renamed, rty) of
       (SomeSymbol proxy, This ty) ->
          case fieldRequiredness of
            Default -> pure . This $ SField proxy fieldName ty fieldVal schema
