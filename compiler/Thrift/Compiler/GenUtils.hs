...
genStructConst name struct = RecConstr () (nameToQName name) (fields struct)
  where
    fields :: StructVal Haskell s -> [FieldUpdate ()]
    fields Empty = []
    fields (ConsVal proxy ty val s) =
      FieldUpdate () (getName proxy) (genConst ty val) : fields s
    fields (ConsDefault proxy ty defVal s) =
      FieldUpdate () (getName proxy)
      (maybe (typeToDefault ty) (genConst ty) defVal) : fields s
    fields (ConsJust proxy ty val s) =
      FieldUpdate () (getName proxy)
        (qcon "Prelude" "Just" `app` genConst ty val) : fields s
    fields (ConsNothing proxy s) =
      FieldUpdate () (getName proxy) (qcon "Prelude" "Nothing") : fields s
...
qualifyStruct q (ConsDefault pr ty val s) =
  ConsDefault pr (qualifyType q ty) (qualifyConst q ty <$> val) $
  qualifyStruct q s
