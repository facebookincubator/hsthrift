...
import qualified G.Types as Types
...
data TU__ServiceCommand a where
  DoNothing :: Types.Foo -> TU__ServiceCommand ()
...
reqName' (DoNothing __field__f) = "doNothing"
...
reqParser' _proxy "doNothing"
  = ST.runSTT
      (do Prelude.return ()
          __field__f <- ST.newSTRef Default.def
          let
            _parse _lastId
                     ...
                     Thrift.FieldBegin _type _id _bool -> do case _id of
                                                               1 | _type ==
                                                                   ...
                                                                   do !_val <- Trans.lift
                                                                                 (Thrift.parseStruct
                                                                                    _proxy)
                                                                      ST.writeSTRef __field__f _val
                                                               _ -> Trans.lift
                                                                      (Thrift.parseSkip _proxy _type
                                                                         (Prelude.Just _bool))
                                                             _parse _id
                     Thrift.FieldEnd -> do !__val__f <- ST.readSTRef __field__f
                                           Prelude.pure (Thrift.Some (DoNothing __val__f))
            _idMap = HashMap.fromList [("f", 1)]
          _parse 0)
reqParser' _ funName
