...
module B.Types
       (B(B, b_a, b_b, b_c), C(C, c_x, c_y),
        ...
        set_value, map_value, hash_map_value)
       where
...
data C = C{c_x :: [Number], c_y :: [Number_Strict]}
         deriving (Prelude.Eq, Prelude.Show, Prelude.Ord)

instance Aeson.ToJSON C where
  toJSON (C __field__x __field__y)
    = Aeson.object
        ("x" .= __field__x : "y" .= __field__y : Prelude.mempty)

instance Thrift.ThriftStruct C where
  buildStruct _proxy (C __field__x __field__y)
    = Thrift.genStruct _proxy
