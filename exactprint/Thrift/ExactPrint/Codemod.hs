...
codemodConsts f p@Program{..} = p { progDecls = map codemodDecl progDecls }
  where
    ...
    codemodListVal outer inner c@ListVal{..} = f progEnv outer
    ...
    codemodMapVal outer k v m@MapVal{..} = f progEnv outer
      m { mvElems =
    ...
    codemodSchema schema m@MapVal{..} =
      m { mvElems =
             ...
             | e@ListElem
                 {leElem=
                     mp@MapPair{mpKey=UntypedConst{ucConst=StringVal s _},..}}
                 <- mvElems
             ]
        }
    codemodSchema _ c = c
    ...
    codemodSchemaField (SField _ name ty _ schema) fl c
