...
computeStructOffsets origin Struct{..} =
  ...
  where
    (offsets, anns, structEnd) =
      computeStructLoc origin structLoc structAnns fieldEnd
    (fields, fieldEnd) =
...
computeUnionOffsets origin Union{..} =
  ...
  where
    (offsets, anns, unionEnd) =
      computeStructLoc origin unionLoc unionAnns altsEnd
    (alts, altsEnd) =
...
computeEnumOffsets origin Enum{..} =
  (Enum { enumConstants = consts
   ...
   enumEnd)
  where
    (offsets, anns, enumEnd) = computeStructLoc origin enumLoc enumAnns constEnd
    (consts, constEnd) =
...
enumValOffsets origin EnumValue{..} =
  (EnumValue
    { evLoc = EnumValLoc
      { evlName      = getOffsets origin evlName
...
computeTypedefOffsets origin Typedef{..} =
  (Typedef
    { tdType = ty
    , tdLoc = TypedefLoc
      { tdlKeyword = getOffsets origin tdlKeyword
...
computeConstOffsets origin Const{..} =
  (Const
     ...
     , constVal = val
     , constLoc = ConstLoc
       { clKeyword = getOffsets origin clKeyword
...
constOffsets origin UntypedConst{..} =
  ...
  where
    (c, end) = case ucConst of
      IntVal i r    -> (IntVal i r, ucLoc)
      DoubleVal d r -> (DoubleVal d r, ucLoc)
      StringVal s q -> (StringVal s q, ucLoc)
      IdVal i     -> (IdVal i, ucLoc)
      BoolVal b   -> (BoolVal b, ucLoc)
      ListVal{..} ->
        (ListVal
         { lvElems = elems
...
functionOffsets origin Function{..} =
  (Function
   ...
   , funExceptions = exs
   , funLoc = FunLoc
     { fnlOneway     = getOffsets origin <$> fnlOneway
   ...
   sepEnd)
  where
    onewayEnd = fromMaybe origin $ lLocation <$> fnlOneway
