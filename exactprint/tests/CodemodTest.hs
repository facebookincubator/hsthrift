...
convertBools TBool (IntVal 0 _) = BoolVal False
convertBools TBool (IntVal 1 _) = BoolVal True
convertBools _ c = c
...
qualifyEnums env (TEnum enum _loc1 _) (IdVal name)
...
qualifyEnum Name{..} name = IdVal $ Text.intercalate "." elems
  where
...
convertEnums env (TEnum enum _loc _) (IntVal x _)
