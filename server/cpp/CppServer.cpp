...
class CppServer : public ThriftServer {
 public:
  ...
  HsString* FOLLY_NULLABLE go(int fd, int* portPtr) {
    try {
      ...
      // apply TLS defaults
//      facebook::services::TLSConfig::applyDefaultsToThriftServer(*this);
      ...
      *portPtr = 0;
      auto exStr = e.what();
      LOG(FATAL) << "Unable to start serving traffic: " << exStr;
...
CreateCppServerResult* c_create_cpp_server(
  ...
  try {
    ...
    auto exStr = e.what();
    LOG(FATAL) << "Failed to start server: " << exStr;
...
HsString* c_serve_cpp_server(
    apache::thrift::CppServer* s,
    int fd,
    int* portPtr) noexcept {
  return s->go(fd, portPtr);
}
...
void c_stop_cpp_server(apache::thrift::CppServer* s) noexcept {
  try {
    ...
    LOG(FATAL) << "Error stopping server: " << e.what();
  }
}
}
