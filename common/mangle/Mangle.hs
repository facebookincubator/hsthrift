...
itaniumMangle csymbol clen = do
  ...
  either (fail $ "itaniumMangle failed: " ++ symbol) newCString $ mangle symbol
...
showName (Unqual name _) args = if null args
  then lengthEncode "" args [name]
  else concat [name, "I", args, "E"]
