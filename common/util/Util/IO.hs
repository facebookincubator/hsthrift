...
-- https://fburl.com/codex/05mqipn2). Like getUserForProcess, but works on on
-- demand.
getDevserverUser :: IO String
getDevserverUser = do
  user <- getUserForProcess
  if user `elem` ["svcscm", "twsvcscm"]
  then do
    owners <- rightToMaybe <$> tryIOError (readFile "/etc/devserver.owners")
    return $ fromMaybe user (listToMaybe =<< (lines <$> owners))
  else
    return user
