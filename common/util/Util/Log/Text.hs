...
vlog :: (MonadIO m, ?s :: CallStack) => Int -> Text -> m ()
...
logInfo :: (MonadIO m, ?s :: CallStack) => Text -> m ()
logInfo msg = liftIO $ logCommon c_glog_info (getCaller $ getCallStack ?s) msg

-- | Provided to support the new logging API. Please use @LOG_INFO()@
-- macro for this facility.
logWarning :: (MonadIO m, ?s :: CallStack) => Text -> m ()
...
-- macro for this facility.
logError :: (MonadIO m, ?s :: CallStack) => Text -> m ()
...
logCritical :: (MonadIO m, ?s :: CallStack) => Text -> m ()
logCritical msg =
