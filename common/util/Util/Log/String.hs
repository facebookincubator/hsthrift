...
vlog :: (MonadIO m, ?s :: CallStack) => Int -> String -> m ()
vlog level msg = liftIO $ do
  b <- vlogIsOn level
  when b $ logCommon c_glog_verbose (getCaller $ getCallStack ?s) msg
...
logInfo :: (MonadIO m, ?s :: CallStack) => String -> m ()
logInfo msg = liftIO $ logCommon c_glog_info (getCaller $ getCallStack ?s) msg

-- | Provided to support the new logging API. Please use @LOG_WARNING()@
-- macro for this facility.
logWarning :: (MonadIO m, ?s :: CallStack) => String -> m ()
logWarning msg = liftIO $ logCommon c_glog_warning
  (getCaller $ getCallStack ?s) msg

-- | Provided to support the new logging API. Please use @LOG_ERROR()@
-- macro for this facility.
logError :: (MonadIO m, ?s :: CallStack) => String -> m ()
logError msg = liftIO $ logCommon c_glog_error (getCaller $ getCallStack ?s) msg

-- | Provided to support the new logging API. Please use
-- @LOG_CRITICAL()@ macro for this facility.
logCritical :: (MonadIO m, ?s :: CallStack) => String -> m ()
logCritical msg =
  liftIO $ logCommon c_glog_critical (getCaller $ getCallStack ?s) msg
